...
自分メモ
	Flickeyのインタラクションを参考に

	アンケートの分析方法違うのでは？ノンパラ　FaceFlexのときの

付箋メモ
	InvisibleFlick
		境界線も最後はインビジブルが良かったりする？
...
1. ノギスを用いて左手の人差し指の幅[^finger]（図[@fig:finger]）を測定した．平均サイズは14.9mm（$SD$=1.4mm）であり，日本人の標準的なサイズの範囲内であった[@hand]．測定後，スマートフォンをマジックテープを用いて左手に装着した．
...
![各手法の各セッションにおける平均修正済みエラー率（Cerr）](../assets/resultCerr.png){#fig:result-cerr width=1.0\\linewidth}
...
![各手法に対する印象の5段階評価の平均値](../assets/ex-five.png){#fig:ex-five width=1.0\\linewidth,height=0.3\\paperheight}
...
- ダイヤル（竜頭）を用いる方法
- Apple Watch[^apple-watch]に搭載されているような圧力検知可能なタッチスクリーンを用いる方法
- テンキー部分の外からスライドインする方法
- 2本の指でマルチタッチジェスチャを行う方法

1つ目と2つ目は現状のタッチ操作をそのまま利用することが可能であるという利点がある．
一方，3つ目と4つ目はすべての操作がタッチで完結するという利点がある．
...
- 腕の外側（手の甲側）に画面が向いているため，タッチ操作するためには手首を画面が見える顔側にひねる必要がある．
- 端末を装着している側の脇を開き，腕を空中に上げたまま維持する必要がある．
- 傾きのある画面でタッチ操作するため，操作する腕を動かし続ける必要がある．
